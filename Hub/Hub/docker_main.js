...
let fs          = require('fs');
let config = JSON.parse(fs.readFileSync('../../Settings/argus/Node.json','utf8'));

if (!config) {
    console.log("Unable to process Node.js config file... it is either missing or has issues");
    exit;
}
...
let http        = require("http").createServer(app).listen(3000,function () {
...
app.post('/emit', function (req,res) {
    console.log('Received data via EMIT');
            ...
            for (var user_id in users) {
                if (data.uid == user_id) {
                    for (var socket_id in users[user_id]) {
                        console.log('Sending '+user_id+' on socket '+socket_id+' the message '+e);
...
//      console.log(data);
    ...
    //
    //--------------------------------------------------------------------------
    socket.on('dentalConsultationSigned',function (data) {
        console.log('A dental consultation form has been signed - '+data.form_id);
        io.emit('dentalFormSigned',data);
    });

    //--------------------------------------------------------------------------
    //
    //--------------------------------------------------------------------------
    socket.on('currentToothCheck',function (data) {
        if (!forms[data.form_id]) {
            forms[data.form_id] = {
                currentTooth: null
            };
        }
        console.log('The current selected tooth is '+forms[data.form_id].currentTooth);
        io.emit('currentTooth',{ 'form_id': data.form_id, 'tooth': forms[data.form_id].currentTooth });
    });
    ...
    socket.on('teledentistryToothPicked',function (data) {
        console.log('A tooth ['+data.tooth+'] has been clicked on in form '+data.form_id);
        if (!forms[data.form_id]) {
            forms[data.form_id] = {
                currentTooth: null
            };
        }
        forms[data.form_id].currentTooth = (data.tooth === forms[data.form_id].currentTooth) ? false : data.tooth;

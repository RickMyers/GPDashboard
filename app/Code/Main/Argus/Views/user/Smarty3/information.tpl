...
<!--
    ...
    on the 'user-information' placeholder with a unique name for the form element you are configuring.
        ...
        <td colspan="3" align="center" valign="middle">
            ...
            <form name="config-user-information-form" id="config-user-information-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    Please enter the name of the field that will contain the value (either a user name or email) to use to retrieve the user information, 
                    and enter the name of the field (node) to attach that information to<br /><br />
                    Source: <input class='paradigm-config-form-field' type="text" name="source" id="source_{$data.id}" value="{if (isset($data.source))}{$data.source}{/if}" /><br />
                    Field: <input class='paradigm-config-form-field' type="text" name="field" id="field_{$data.id}" value="{if (isset($data.field))}{$data.field}{/if}" /><br />

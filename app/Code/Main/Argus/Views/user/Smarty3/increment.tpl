...
<!--
    ...
    on the 'node-field' placeholder with a unique name for the form element you are configuring.
...
<!-- ################################ END HEADER SECTION ########################################-->    
    <tr>
        <td colspan="3" align="center" valign="middle">
            <!-- ########################## FORM SECTION ########################################-->
            <form name="config-node-field-form" id="config-node-field-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    Please set the Event node name, and the field name of that node, where we can retrieve the user name to use to increment the number of attempts
                    this user has made to log in.  This is assuming that a previous step has already retrieved the user information and attached it to the Event using
                    the node name below:<br /><br />
                    Node: <input class='paradigm-config-form-field' type="text" name="node" id="node_{$data.id}" value="{if (isset($data.node))}{$data.node}{/if}" /><br /><br >
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-node-field-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>

...
<table style="width: 100%; height: 100%; border-spacing: 1px;">
    ...
    <tr>
        <td colspan="3" align="center" valign="middle">
            <form name="config-csv-834-form" id="config-csv-834-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    Please specify the name of the Event Field that contains the 834 file name we will be processing, and the 
                    name of the Event Field to attach the CSV to<br /><br />
                834 File Event Field: <input class='paradigm-config-form-field' type="text" name="event_field" id="event_field_{$data.id}" value="{if (isset($data.event_field))}{$data.event_field}{/if}" /><br /><br />
                834 CSV Event Field: <input class='paradigm-config-form-field' type="text" name="event_field" id="event_field_{$data.id}" value="{if (isset($data.event_field))}{$data.event_field}{/if}" /><br /><br />
                <br >
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-csv-834-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>

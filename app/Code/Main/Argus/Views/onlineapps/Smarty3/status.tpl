        ...
        <td colspan="3" align="center" valign="middle">
            <form name="config-remoteupdate-form" id="config-remoteupdate-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    You can choose to set the Online Application status to a fixed value or you can pull the value to set the remote status from a field in the event,
                    but then you also need to identify the field on the event where the application data is stored.
                    You can also specify a value for the "Last Action" field on the dashboard queue apps<br /><br />
                ...
                Last Action: <input class='paradigm-config-form-field' type="text" name="last_action" id="last_action_{$data.id}" value="{if (isset($data.last_action))}{$data.last_action}{/if}" />(optional)<br /><br >    

...
{assign var=results value=$query->dynamicQuery()}
<table>
    <tr>
    ...
    <tr style="border-bottom: 1px solid #eee; background-color: rgba(99,99,99,{cycle values=".1,.3"})">
        <td class="query-cell"><img src="/images/dashboard/edit_icon.png" class="query-icon-{$window_id}" row_id="{if (isset($row.uid))}{$row.uid}{else}{$row.id}{/if}" /> </td>
...
<script type="text/javascript">
    ...
    (function () {
        ...
        EasyEdits.populateSelectBox('page_{$window_id}',pages);
        $('#page_{$window_id}').val({$query->_page()});
    })();
</script>
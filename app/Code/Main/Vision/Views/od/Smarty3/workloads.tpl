...
<!-- ################################ END HEADER SECTION ########################################-->    
    <tr>
        <td colspan="3" align="center" valign="middle">
            <!-- ########################## FORM SECTION ########################################-->
            <form name="config-od-workloads-form" id="config-od-workloads-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    We are going to be populating a list of present work items, in both the staging and submitted queue, for our ODs.
                    <br /><br />
                    This form lets you specify what the name of the field on the event we will attach this data to. <br /><br />
                    Event Field Name: <input class='paradigm-config-form-field' type="text" name="field_name" id="field_name_{$data.id}" value="{if (isset($data.field_name))}{$data.field_name}{/if}" /><br /><br >
                    <br />
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-od-workloads-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>

...
<!--
    ...
    on the 'ipa-attach' placeholder with a unique name for the form element you are configuring.
...
<!-- ################################ END HEADER SECTION ########################################-->    
    <tr>
        <td colspan="3" align="center" valign="middle">
            <!-- ########################## FORM SECTION ########################################-->
            <form name="config-ipa-attach-form" id="config-ipa-attach-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    Specify the field name that will contain the IPA ID to use in the lookup, and the name of the field to attach the lookup information to<br /><br />
                    
                    IPA ID: <input class='paradigm-config-form-field' type="text" name="ipa_id_name" id="ipa_id_name_{$data.id}" value="{if (isset($data.ipa_id_name))}{$data.ipa_id_name}{/if}" /><br /><br >
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-ipa-attach-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>

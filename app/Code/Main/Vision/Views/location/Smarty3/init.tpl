<style type='text/css'>
    ...
    .location_pagination_control {
        padding: 2px 5px; font-size: .8em
    }    
</style>
<div class='location_queue_body'>
        ...
        <div class="nc_heading"  style="width: 3%">
           &diams;
            ...
            <select name="health_plan" id="health_plan"  style='background-color: yellow; color: #333'>
                ...
                {foreach from=$pcps->setLocationId({$location.id})->healthPlans() item=hp key=id}
                    <option value="{$id}">{$hp}</option>
                {/foreach}
            </select>
            ...
            <select name="physician_npi" id="physician_npi" style='background-color: yellow; color: #333'>
                ...
                {foreach from=$pcps->setLocationId({$location.id})->physicians() item=pcp key=npi}
                    <option value="{$npi}">Dr. {$pcp}</option>
                {/foreach}
            </select>
        </div><div class="nc_heading" onclick="Argus.vision.location.sortField('technician_name')" style="width: 11%">Technician</div>    
    </div>    
    <div id="location_clients_queue"></div>
</div>
<div class='location_queue_footer'>
    <table width='100%'>
        <tr>
            <td>
                <span id='location_clients-from-row'></span>-<span id='location_clients-to-row'></span> of <span id='location_clients-rows'></span>
            </td>
            <td align='center'>
                <input type='button' name='location_clients-previous' id='location_clients-previous'  style='' class='location_pagination_control' value='<' />
                <input type='button' name='location_clients-first' id='location_clients-first' style='' class='location_pagination_control' value='<<' />
                &nbsp;&nbsp;
                <input type='button' name='location_clients-last' id='location_clients-last' style='' class='location_pagination_control' value='>>' />
                <input type='button' name='location_clients-next' id='location_clients-next' style='' class='location_pagination_control' value='>' />
            </td>
            <td align='right' style='padding-right: 4px'>
                <span id='location_clients-page'></span> of <span id='location_clients-pages'></span>
            </td>
        </tr>
    </table>
</div>
<script type="text/javascript">
    Pagination.init('location_clients',function (page,rows) {
        Argus.vision.location.refresh($E('location_clients_queue'),'location_clients',page,rows);
    },1,50);
    $('#physician_npi').on('change',function (evt) {
        Argus.vision.location.refresh($E('location_clients_queue'),'location_clients',1,50)
    });

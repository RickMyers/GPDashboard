...
<table style="width: 100%; height: 100%; border-spacing: 1px;">
    ...
    <tr>
        <td colspan="3" align="center" valign="middle">
            <form name="config-unassigned-field-form" id="config-unassigned-field-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    This process will sum up the number of vision scanning records that have a status of "submitted" and no assigned reviewer (O.D.).<br /><br />
                    Please enter the name of the field to store this information:<br /><br/>
                    Field: <input class='paradigm-config-form-field' type="text" name="field" id="config_unassigned_field_{$data.id}" value="{if (isset($data.field))}{$data.field}{/if}" /><br /><br >
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-unassigned-field-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>
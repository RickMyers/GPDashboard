...
<script type="text/javascript">
    ...
    $('#search-page-search-button-{$window_id}').on('click',function (evt) {
        if ($('#search-page-search-field-{$window_id}').val()) {
            Argus.vision.search.execute($('#search-page-search-field-{$window_id}').val(),'{$window_id}',1,20);
        }
    ...
    Pagination.init('vision-search-{$window_id}',function (page,rows) {
        Argus.vision.search.execute($('#search-page-search-field-{$window_id}').val(),'{$window_id}',page,rows);
    },1,20); 
</script>

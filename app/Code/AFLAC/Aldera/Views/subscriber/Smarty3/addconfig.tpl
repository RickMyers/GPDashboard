...
<table style="width: 100%; height: 100%; border-spacing: 1px;">
    ...
    <tr>
        <td colspan="3" align="center" valign="middle">
            <form name="config-addsubscriber-form" id="config-addsubscriber-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    <br />
                    Enter the name of the field on the event that contains the subscriber data<br /><br />
                    Event Field <input class='paradigm-config-form-field' type="text" name="field" id="config_addsubscriber_{$data.id}" value="{if (isset($data.field))}{$data.field}{/if}" /><br /><br >
                    Name of field on the event to put the results of the Subscriber/Member add<br /><br />
                    Result Field <input class='paradigm-config-form-field' type="text" name="result" id="config_addsubscriber_result_{$data.id}" value="{if (isset($data.result))}{$data.result}{/if}" /><br /><br >
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-addsubscriber-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>
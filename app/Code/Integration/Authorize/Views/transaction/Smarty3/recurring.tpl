...
<table style="width: 100%; height: 100%; border-spacing: 1px;">
    ...
    <tr>
        <td colspan="3" align="center" valign="middle">
            <form name="config-recurring-form" id="config-recurring-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    Identify the field below on the event that contains the information required to schedule recurring billing with Authorize.net as well as the event field to store the reply back from Authorize.net<br /><br />
                    <table>
                        <tr>
                    ...
                    Source Field: <input class='paradigm-config-form-field' type="text" name="field" id="config_recurring_field_{$data.id}" value="{if (isset($data.field))}{$data.field}{/if}" /><br /><br />
                    Result Field: <input class='paradigm-config-form-field' type="text" name="result" id="config_recurring_result_field_{$data.id}" value="{if (isset($data.result))}{$data.result}{/if}" /><br />
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-recurring-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>

...
<table style="width: 100%; height: 100%; border-spacing: 1px;">
    ...
    <tr>
        <td colspan="3" align="center" valign="middle">
            <form name="config-subscription-update-form" id="config-subscription-update-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    Please specify the name of the field on the event that contains the result from the attempt to update Authorize.net with data:<br /><br />
                    <table>
                        <tr><td>Subscription Update Results Field </td><td>&nbsp;<input class='paradigm-config-form-field' type="text" name="field" id="config_event_field_{$data.id}" value="{if (isset($data.field))}{$data.field}{/if}" /><br /></td></tr>
                    </table>
                    <br />
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-subscription-update-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>

...
<table style="width: 100%; height: 100%; border-spacing: 1px;">
    ...
    <tr>
        <td colspan="3" align="center" valign="middle">
            <form name="config-subscribed-form" id="config-subscribed-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    Identify the field below on the event that contains the information returned by Authorize.net that has the results of the attempt to set up subscribed billing<br /><br />
                    Field: <input class='paradigm-config-form-field' type="text" name="field" id="config_subscribed_field_{$data.id}" value="{if (isset($data.field))}{$data.field}{/if}" />
                    <br /><br />                    
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-subscribed-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>

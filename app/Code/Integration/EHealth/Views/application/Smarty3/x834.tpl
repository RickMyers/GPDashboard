...
<table style="width: 100%; height: 100%; border-spacing: 1px;">
    ...
    <tr>
        <td colspan="3" align="center" valign="middle">
            <form name="config-x834-form" id="config-x834-form-{$data.id}" onsubmit="return false">
                ...
                <fieldset style="padding: 10px; width: 600px; text-align: left"><legend>Instructions</legend>
                    Please specify the field in the event to attach the generated 834 file to.  If you know the Aldera plan, you can specify it also<br /><br />
                Event Field: <input class='paradigm-config-form-field' type="text" name="field" id="config_x834_event_field_{$data.id}" value="{if (isset($data.field))}{$data.field}{/if}" /><br /><br >
                Aldera Plan: <input class='paradigm-config-form-field' type="text" name="plan"  id="config_x834_plan_{$data.id}" value="{if (isset($data.plan))}{$data.plan}{/if}" /><br /><br >
...
<script type="text/javascript">
    ...
    Form.intercept($('#config-x834-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$windowId}");
</script>
